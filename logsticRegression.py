...
x_data = np.loadtxt('D:\Learning_code\TensorFlow_\data\ex4x.dat').astype(np.float32)
y_data = np.loadtxt('D:\Learning_code\TensorFlow_\data\ex4y.dat').astype(np.float32)
# x_data.shape (80,2)
# y_data.shape(80,)
...
# we evalue the x and y sklearn to get a sense of the coefficients
reg = LogisticRegression(C=99999, solver="newton-cg") # Set C as a large positive number to minimize the regularization effect
reg.fit(x_data, y_data)
print("Coefficients of sklearn: K=%s, b=%f" % (reg.coef_, reg.intercept_))

# Now we use tensorflow to get the similar results
...
reg1 = LogisticRegression(C=999999, solver='newton-cg', multi_class="multinomial")
reg1.fit(x_data, y_data)
print("Coefficients of sklearn: K=%s, b=%f" % (reg1.coef_, reg1.intercept_))
print("A little bit difference at first glance. What about multiply them with 2?")

# then we try tensorflow
...
# tensorflow provide a utility function to calculate the probability of observer n choose alternative i, you can replace it with `y = tf.exp(V) / tf.reduce_sum(tf.exp(V), keep_dims=True, reduction_indices=[1])`
y = tf.nn.softmax(V)
...
# 对于逻辑回归，损失函数比线性回归模型复杂了一些。首先需要通过sigmoid函数，将线性回归的结果转化为0至1之间的概率值。然后写出每个样本的发生概率（似然），
# 那么所有样本的发生概率就是每个样本发生概率的乘积。为了求导方便，我们对所有样本的发生概率取对数，保持其单调性的同时，可以将连乘变为求和（加法的求导公式比乘法的求导公式简单很多）。
# 对数极大似然估计方法的目标函数是最大化所有样本的发生概率；机器学习习惯将目标函数称为损失，所以将损失定义为对数似然的相反数，以转化为极小值问题。
